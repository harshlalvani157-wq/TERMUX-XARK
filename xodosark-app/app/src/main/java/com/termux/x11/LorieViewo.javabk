...
public class LorieView extends SurfaceView implements InputStub {
    ...
    private MainActivity termuxMainActivity() {
    ...
    private final InputConnection mConnection = new InputConnectionWrapper(new BaseInputConnection(this, false) {
        ...
        boolean replaceText(CharSequence newText, boolean reuse) {
            ...
            MainActivity ma = LorieView.this.termuxMainActivity();
            if (ma != null && ma.useTermuxEKBarBehaviour && ma.mExtraKeys != null)
                ma.mExtraKeys.unsetSpecialKeys();
            commitedText = true;
            return true;
        }
    ...
    public void triggerCallback() {
        ...
        android.app.Activity host = X11ActivityBridge.getHostActivity();
        if (host != null) {
            // Never run surfaceChanged inline on the UI stack: it enters native code and can block,
            // freezing Compose immediately after the user taps "Container".
            new Handler(Looper.getMainLooper()).post(
                    () -> mSurfaceCallback.surfaceChanged(getHolder(), PixelFormat.BGRA_8888, r.width(), r.height())
            );
        }
    }

    void getDimensionsFromSettings() {
        Prefs prefs = X11ActivityBridge.getOrCreatePrefs(getContext());
    ...
    public boolean dispatchKeyEventPreIme(KeyEvent event) {
        ...
        return X11ActivityBridge.handleKey(event);
    }
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        if (X11ActivityBridge.getOrCreatePrefs(getContext()).enforceCharBasedInput.get())
